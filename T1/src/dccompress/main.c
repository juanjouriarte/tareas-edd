...
int main(int argc, char** argv) {
	...
	Quadtree* tree = quadtree_from_image(img);
	...
	/* Si estamos en modo comprimir */
		...
		alpha = find_alpha_who_gives_n_leaves_using_binary_search(tree, h);
	}

	/* Una vez definido el alpha, podemos filtrar la imagen */
	quadtree_filter_recursive(tree, alpha);

	quadtree_to_image(tree, img);

	// En vez de pintar un cuadrado, pinta los nodos del árbol //
	printf("alpha: %f\n", alpha);
